...
class model2(ActorCriticPolicy):
    ...
    def _build_mlp_extractor(self) -> None:
        self.mlp_extractor = DOOM2(self.features_dim)


class DOOM3(nn.Module):
    def __init__(
        self,
        feature_dim: int,
        last_layer_dim_pi: int = 64,
        ...
        # self.attention = nn.MultiheadAttention(embed_dim=128, num_heads=4, batch_first=True)
        self.lstm = nn.LSTM(128, 128, 4, batch_first=True)
...
class model3(ActorCriticPolicy):
    ...
    def _build_mlp_extractor(self) -> None:
        self.mlp_extractor = DOOM3(self.features_dim)
